"""Home Assistant Temperature Web Application - Main Application Factory."""
...
try:
    ...
    logger.warning("Invalid REFRESH_INTERVAL_SECONDS, using default: 15")
    REFRESH_INTERVAL_SECONDS = 15

try:
    HISTORY_POINTS = int(os.getenv("HISTORY_POINTS", "100"))
except ValueError:
    logger.warning("Invalid HISTORY_POINTS, using default: 100")
    HISTORY_POINTS = 100

try:
    REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "10"))
except ValueError:
    logger.warning("Invalid REQUEST_TIMEOUT, using default: 10")
    REQUEST_TIMEOUT = 10
...
class Config:
    def __init__(self):
        self.is_multi_entity_mode = bool(ENTITIES)
        self.primary_entity = ENTITY_ID or (ENTITIES[0] if ENTITIES else None)
        self.REFRESH_INTERVAL_SECONDS = REFRESH_INTERVAL_SECONDS
        self.HISTORY_POINTS = HISTORY_POINTS
    
    def get_entities_to_query(self):
        """Get list of entities to query."""
        if ENTITIES:
            return ENTITIES
        elif ENTITY_ID:
            return [ENTITY_ID]
...
# Template for single-entity display
template = """
<!DOCTYPE html>
<html lang="en">
<head>
...
# Dashboard template with auto-refresh and sparklines
dashboard_template = """
<!DOCTYPE html>
<html lang="en">
<head>
...
def get_sensor_reading(entity_id: str) -> Tuple[Optional[float], Optional[str], Dict[str, str]]:
    """Get sensor reading from Home Assistant or return dummy data."""
    if USE_DUMMY_DATA:
...
def get_backyard_temperature() -> Tuple[Optional[float], Optional[str]]:
    """Get temperature from Home Assistant or use dummy data (legacy function)."""
    if USE_DUMMY_DATA:
    ...
    temp_c, last_updated, _ = get_sensor_reading(target_entity)
    return temp_c, last_updated


def _append_history(entity_id: str, value_c: float) -> None:
    """Append a value to the history buffer for an entity."""
    if value_c is None:
        return
...
@app.route('/')
def index():
    """Display the temperature on the main page."""
...
@app.route('/dashboard/')
def dashboard():
    """Display the multi-entity dashboard."""
...
def api_sensors():
    """Return current readings and history for multi-entity dashboard."""
...
def health():
    ...
    return jsonify({
        ...
        "timestamp": int(time()),
        "version": "1.0.0",
        "entities_count": len(ENTITIES if ENTITIES else ([ENTITY_ID] if ENTITY_ID else [])),
        "multi_entity_mode": bool(ENTITIES)
    })
...
if __name__ == '__main__':
    ...
    app.run(host=FLASK_RUN_HOST or "0.0.0.0", port=FLASK_RUN_PORT or 5000)